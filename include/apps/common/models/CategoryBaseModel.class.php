...
class CategoryBaseModel extends BaseModel {
    ...
    function get_categories_tree($cat_id = 0) {
        ...
        if ($this->row($sql) || $parent_id == 0) {
            /* 获取当前分类及其子分类 */
             ...
             $sql = 'SELECT c.cat_id,c.cat_name,c.parent_id,c.is_show,t.cat_image as cat_img ' .
                    'FROM ' . $this->pre . 'category as c ' .
                    'left join  ' . $this->pre . 'touch_category t on t.cat_id = c.cat_id '.
    ...
    function get_child_tree($tree_id = 0) {
        ...
        if ($this->row($sql) || $tree_id == 0) {
            $child_sql = 'SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show,t.cat_image as cat_img ' .
                    'FROM ' . $this->pre . 'category as c ' .
                    'left join  ' . $this->pre . 'touch_category t on t.cat_id = c.cat_id '.
